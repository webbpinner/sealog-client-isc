    ...
    if(this.props.event.events && this.props.event.events.length > 0){
      ...
      let eventList = this.props.event.events.map((event, index) => {
        if(index >= (this.state.activePage-1) * maxEventsPerPage && index < (this.state.activePage * maxEventsPerPage)) {
          ...
          return (<ListGroupItem key={event.id} active={active} ><Row><Col xs={11} onClick={() => this.handleEventClick(index)}>{event.ts} {`<${event.event_author}>`}: {event.event_value} {eventOptions}</Col><Col>{commentTooltip}</Col></Row></ListGroupItem>);
        }
      })
      return eventList
    }
