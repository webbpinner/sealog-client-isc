...
class SetLoweringStatsModal extends Component {
  ...
  constructor (props) {
    ...
    this.auxDatasourceFilters = ['realtimeNavData'];
  ...
  async initLoweringTrackline() {
    ...
    if(tracklines.realtimeNavData) {
      this.setState((prevState) => { return { events: events, tracklines: tracklines, fetching: false, depthChartOptions: Object.assign(prevState.depthChartOptions, { series: [ { data: tracklines.realtimeNavData.depth } ] }) } });
    }
    else {
  ...
  handleCalculateBoundingBox() {
    if(this.state.tracklines.realtimeNavData && !this.state.tracklines.realtimeNavData.polyline.isEmpty()) {
      let lowering_bounds = this.state.tracklines.realtimeNavData.polyline.getBounds()
  ...
  handleCalculateMaxDepth() {
    ...
    if(this.state.tracklines.realtimeNavData && this.state.tracklines.realtimeNavData.depth.length > 0) {
      let maxDepth = this.state.tracklines.realtimeNavData.depth.reduce((current_max_depth, depth) => {
  ...
  //     let maxDepthPair = this.state.tracklines.realtimeNavData.depth.find((depth) => depth[1] === this.state.stats.max_depth)
  //     if(maxDepthPair) {
  //       xAxis.plotLines.push({
  //           color: '#FF0000',
  //           width: 2,
  ...
  renderMarker() {
    ...
    if(this.state.event) {
      ...
      const realtimeNavData = this.state.event.aux_data.find((data) => data['data_source'] === 'realtimeNavData');
      return (
  ...
  render() {
    ...
    // if (this.state.tracklines.realtimeNavData && this.state.tracklines.realtimeNavData.depth) {
    //   console.log("depth:", this.state.tracklines.realtimeNavData.depth)
    // }
