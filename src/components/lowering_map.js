...
class LoweringMap extends Component {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      zoom: 14,
      center:{lat:41.492233349, lng:-71.422483921},
      position:{lat:41.492233349, lng:-71.422483921},
      showMarker: false,
      height: "480px"
    };

    this.auxDatasourceFilters = ['realtimeNavData', 'vehicleReNavData'];
  ...
  initMapView() {
  	...
  	else if(this.state.tracklines.realtimeNavData && !this.state.tracklines.realtimeNavData.polyline.isEmpty()) {
  		this.map.leafletElement.panTo(this.state.tracklines.realtimeNavData.polyline.getBounds().getCenter());
  		this.map.leafletElement.fitBounds(this.state.tracklines.realtimeNavData.polyline.getBounds());
  	}
  }
  ...
  calcVehiclePosition(selected_event) {
    if(selected_event && selected_event.aux_data) {
      let realtimeNavData = selected_event.aux_data.find(aux_data => aux_data.data_source == "realtimeNavData");
      if(realtimeNavData) {
        let latObj = realtimeNavData.data_array.find(data => data.data_name == "latitude");
        let lonObj = realtimeNavData.data_array.find(data => data.data_name == "longitude");
    ...
    if(this.props.event.events && this.props.event.events.length > 0){
      ...
      let eventList = this.props.event.events.map((event, index) => {
        if(index >= (this.state.activePage-1) * maxEventsPerPage && index < (this.state.activePage * maxEventsPerPage)) {
          ...
          if(edu_event) {
            permalinkTooltip = (seatube_exists)? (<OverlayTrigger placement="top" overlay={<Tooltip id={`commentTooltip_${event.id}`}>Open Seatube Permalink</Tooltip>}><a href={seatube_permalink} target="_blank"><FontAwesomeIcon icon='link' className={(!active)? "text-primary" : "text-secondary" } fixedWidth/></a></OverlayTrigger>) : null
          ...
          return (<ListGroup.Item className="event-list-item" key={event.id} active={active} ><span onClick={() => this.handleEventClick(index)} >{`${event.ts} <${event.event_author}>: ${event.event_value} ${eventOptions}`}</span><span className="float-right">{youtubeTooltip} {permalinkTooltip} {eventDetails} {eventComment} </span></ListGroup.Item>);
  ...
  renderMarker() {
  	...
  	if(this.props.event.selected_event.aux_data && typeof this.props.event.selected_event.aux_data.find((data) => data['data_source'] === 'realtimeNavData') !== 'undefined') {

  		const realtimeNavData = this.props.event.selected_event.aux_data.find((data) => data['data_source'] === 'realtimeNavData');
  		return (
  ...
  render() {
    ...
    const realtimeTrack = (this.state.tracklines.realtimeNavData && !this.state.tracklines.realtimeNavData.polyline.isEmpty()) ? 
    	<Polyline color="lime" positions={this.state.tracklines.realtimeNavData.polyline.getLatLngs()} />
      : null;
