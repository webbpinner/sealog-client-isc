    ...
    if(this.props.event.events && this.props.event.events.length > 0){
      ...
      let eventList = this.props.event.events.map((event, index) => {
        if(index >= (this.state.activePage-1) * maxEventsPerPage && index < (this.state.activePage * maxEventsPerPage)) {
          ...
          return (<ListGroup.Item className="event-list-item" key={event.id} active={active} ><span onClick={() => this.handleEventClick(event)} >{`${event.ts} <${event.event_author}>: ${event.event_value} ${eventOptions}`}</span><span className="float-right">{youtubeTooltip} {permalinkTooltip} {eventDetails} {eventComment} </span></ListGroup.Item>);
