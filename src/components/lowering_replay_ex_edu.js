...
class LoweringReplay extends Component {
  ...
  constructor (props) {
    ...
    this.postEventUpdate = this.postEventUpdate.bind(this);
  ...
  postEventUpdate(event_id, eventValue, eventFreeText, eventOptions, eventTS) {
    this.props.updateEvent(event_id, eventValue, eventFreeText, eventOptions, eventTS);
  }
      ...
      let return_aux_data = this.props.event.selected_event.aux_data.reduce((filtered, aux_data, index) => {
        if(!excludeAuxDataSources.includes(aux_data.data_source)) {
          ...
          if(aux_data_points.length > 0) {
            filtered.push(
              <Col xs={12} sm={6} md={4} key={`col_${aux_data.data_source}`} style={{paddingBottom: "8px"}}>
    ...
    if(this.props.event.events && this.props.event.events.length > 0){
      ...
      let eventList = this.props.event.events.map((event, index) => {
        if(index >= (this.state.activePage-1) * maxEventsPerPage && index < (this.state.activePage * maxEventsPerPage)) {
          ...
          if(edu_event) {
            // console.log("seatube_exists:", seatube_exists)
            // console.log("seatube_permalink:", seatube_permalink)
          ...
          return (<ListGroup.Item className="event-list-item" key={event.id} active={active} ><span onClick={() => this.handleEventClick(index)} >{`${event.ts} <${event.event_author}>: ${event.event_value} ${eventOptions}`}</span><span className="float-right">{youtubeTooltip} {permalinkTooltip} {eventComment} </span></ListGroup.Item>);
