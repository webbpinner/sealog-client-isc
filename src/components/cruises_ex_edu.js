...
import CreateCruise from './create_cruise_ex_edu';
import UpdateCruise from './update_cruise_ex_edu';
  ...
  renderCruises() {
    ...
    return this.props.cruises.map((cruise, index) => {
      if(index >= (this.state.activePage-1) * maxCruisesPerPage && index < (this.state.activePage * maxCruisesPerPage)) {
        ...
        let mtuID = (MTU && cruise.cruise_additional_meta.mtu_id)? <span>MTU ID: {cruise.cruise_additional_meta.mtu_id}<br/></span> : null

        return (
          <tr key={cruise.id}>
            <td className={(this.props.cruiseid == cruise.id)? "text-warning" : ""}>{cruise.cruise_id}</td>
            <td>{cruiseName}{cruiseVessel}{cruiseLocation}{mtuID}Dates: {moment.utc(cruise.start_ts).format('L')}<FontAwesomeIcon icon='arrow-right' fixedWidth/>{moment.utc(cruise.stop_ts).format('L')}</td>
            <td>
