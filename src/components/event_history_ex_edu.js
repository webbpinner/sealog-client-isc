...
class EventHistory extends Component {
  ...
  constructor (props) {
    ...
    this.handleEventShowDetailsModal = this.handleEventShowDetailsModal.bind(this)
  }
  ...
  renderEventHistory() {
    ...
    if(this.props.history && this.props.history.length > 0){
      ...
      for (let i = 0; i < this.props.history.length; i++) {
        ...
        if(edu_event) {
          permalinkTooltip = (seatube_permalink)? (<OverlayTrigger placement="top" overlay={<Tooltip id={`commentTooltip_${event.id}`}>Open Seatube Permalink</Tooltip>}><a href={seatube_permalink} target="_blank"><FontAwesomeIcon icon='link' fixedWidth /></a></OverlayTrigger>) : null
        ...
        eventArray.push(<ListGroup.Item className="event-list-item" key={event.id}><span onClick={() => this.handleEventShowDetailsModal(event)}>{event.ts} {`<${event.event_author}>`}: {event.event_value} {eventOptions}</span><span className="float-right">{youtubeTooltip} {permalinkTooltip} {commentTooltip} </span></ListGroup.Item>);
      }
      return eventArray;
    }
